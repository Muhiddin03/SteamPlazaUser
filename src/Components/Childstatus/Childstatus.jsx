  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(pickupDocRef, (docSnap) => {
      if (docSnap.exists()) {
        ...
        setChildName(data.studentName || ''); // ✅ to‘g‘ri field nomi
      } else {
      ...
      case 'pending':
        return (
          <>
            ...
            <p className="text-gray-600 mt-2">Tarbiyachining javobini kuting...</p>
            <p className="text-lg font-medium mt-4">{childName} ni olib ketish so'rovi</p>
          </>
        );
      case 'accepted':
        return (
      ...
      case 'rejected':
        return (
          <>
            ...
            <p className="text-gray-600 mt-2">Iltimos, tarbiyachi bilan bog'laning yoki qayta urinib ko'ring.</p>
            <p className="text-lg font-medium mt-4">{childName} ni olib ketish so'rovi</p>
          </>
        );
      case 'not-found':
